...
# Keep the archived prototype self-contained when it is launched from the repo root.
archive_dir = Path(__file__).resolve().parent

# Read the legacy result template.
with open(archive_dir / "breakdown-template.html", "r") as f:
    html_template = f.read()

# Load the API key from the environment. Never keep credentials in source code.
gemini_api_key = os.environ.get("GEMINI_API_KEY")
if not gemini_api_key:
    st.error("Set the GEMINI_API_KEY environment variable to use the legacy Streamlit prototype.")
    st.stop()
...
json_file_path = archive_dir / "saved_sentences.json"
